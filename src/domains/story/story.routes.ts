...
router
  .route('/')
  .get(story_controller.getAllStories)
  .post(
    joiValidationMiddleware(storyPostSchema),
    joiFileValidationMiddleware(fileSchema),
    story_controller.postStory,
  );

router
  .route('/:id')
  .get(story_controller.getStoryById)
  .patch(
    joiValidationMiddleware(storyPostSchema),
    story_controller.patchStory,
  )
