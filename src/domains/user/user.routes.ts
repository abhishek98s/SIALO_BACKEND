...
router.patch(
  '/profilePicture',
  upload.single('sialo_profile_image'),
...
router.patch(
  ...
  joiFileValidationMiddleware(fileSchema),
  verifyToken,
  user_controller.updateCoverPicture,
);
...
router
  ...
  .patch(
    joiValidationMiddleware(changeUsernameSchema),
    user_controller.updateUsername,
  );
router.get('/', user_controller.fetchAll);

router
  .patch('/friend/add/:friendId', user_controller.addFriend)
